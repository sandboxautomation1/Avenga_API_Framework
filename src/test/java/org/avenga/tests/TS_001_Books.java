...
public class TS_001_Books extends BaseTest {
    ...
    @Test(testName = "[TC_001_Books] Retrieve a list of all books", description = "Ensure the API returns accurate HTTP status codes for valid GET request.", groups = { "smoke" }, priority = 0)
    public void TC_001_GetAllBooks() throws Exception {
        books.GET_AllBooks(200).then().body("size()", greaterThan(0));;
    }


    @Test(testName = "[TC_002_Books] Retrieve details of a specific book by its ID", description = "Validate that READ operation work as expected with valid ID.", groups = { "smoke", "acceptance" }, priority = 1)
    ...
    @Test(testName = "[TC_003_Books] Add a new book to the system", description = "Validate that CREATE operation work as expected with valid payload.", groups = { "smoke", "acceptance" }, priority = 2)
    ...
    @Test(testName = "[TC_004_Books] Update an existing book by its ID", description = "Validate that UPDATE operation work as expected with valid payload.", groups = { "smoke", "acceptance" }, priority = 3)
    ...
    @Test(testName = "[TC_005_Books] Delete a book by its ID", description = "Validate that DELETE operation work as expected with valid ID.", groups = { "smoke", "acceptance" }, priority = 4)
    public void TC_005_DeleteBook() throws Exception {
        books.DELETE_DeleteBookById(200, "1");
    }


    @Test(testName = "[TC_006_Books] Retrieve a book with invalid ID", description = "Validate that READ operation work as expected with invalid ID.", groups = { "regression" }, priority = 5)
    public void TC_006_GetBookWithInvalidId() throws Exception {
        books.GET_BookById(404, 201);
    }


    @Test(testName = "[TC_007_Books] Add a new book to the system with missing publishing date", description = "Validate that CREATE operation work as expected with invalid payload.", groups = { "regression" }, priority = 6)
    ...
    @Test(testName = "[TC_008_Books] Update book with missing publishing date", description = "Validate that UPDATE operation work as expected with invalid payload.", groups = { "regression" }, priority = 7)
    ...
    @Test(testName = "[TC_009_Books] Delete non existing record", description = "Validate that DELETE operation work as expected with invalid payload.", groups = { "regression" }, priority = 8)

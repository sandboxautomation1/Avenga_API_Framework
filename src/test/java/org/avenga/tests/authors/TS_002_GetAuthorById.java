...
public class TS_002_GetAuthorById extends BaseTest {
    ...
    public void TC_009_SchemaValidation() throws Exception {
        AuthorResponse response = authors.GET_AuthorById(200, 1).then().extract().as(AuthorResponse.class);

        assertThat(response.getId(), is(1));
        assertThat(response.getLastName(), notNullValue());
        assertThat(response.getLastName(), notNullValue());
        assertThat(response.getIdBook(), greaterThan(0));
    }
    ...
    public void TC_011_SpecialCharacters() throws Exception {
        authors.GET_AuthorById(400, Integer.parseInt("!@"));
    }

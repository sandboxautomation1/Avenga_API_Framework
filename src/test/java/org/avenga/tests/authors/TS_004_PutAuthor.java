...
public class TS_004_PutAuthor extends BaseTest {
    ...
    @Test(testName = "[TC_022_Authors] Valid update author", description = "Validate that a book record can be successfully updated with response code 200", groups = { "smoke" }, priority = 0)
    public void TC_022_ValidUpdate() throws Exception {
    ...
    @Test(testName = "[TC_023_Authors] Update non existing", description = "Validate that a non existing author record can't be updated with response code 404", groups = { "smoke" }, priority = 1)
    public void TC_023_UpdateNonExisting() throws Exception {
    ...
    @Test(testName = "[TC_024_Authors] Update with invalid book ID", description = "Validate that updating records with null field will return response code 400", groups = { "smoke" }, priority = 2)
    public void TC_024_UpdateWithInvalidBookId() throws Exception {
        JsonObject body = request.buildAuthorPayload(1,-1, DataUtils.getFirstName(), DataUtils.getLastName());
